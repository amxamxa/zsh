...
if [[ -f "$ZDOTDIR/colors/color-schema.yml" ]]; then
  ...
  export LS_COLORS="$(vivid generate generate alabaster_dark)"
  printf "\t${GREEN}󰞷 .... vivid mit generate alabaster_dark ${RESET}\n"
fi
...
echo "        	    __              __              __      
 ...
 #echo "${BOLD}  shift+KP_MULTIPLY	next_layout
  ...
  #echo "  ------------------------------------------------------
  echo "${PINK} fzf documentation: $ZDOTDIR/fzf/README.md${PINK}"
